...
import { Component, Output, EventEmitter, signal, inject, ViewChild, } from '@angular/core';
...
// ─── Componente ───────────────────────────────────────────────────────────────
let DanyChatComponent = class DanyChatComponent {
    constructor() {
        ...
        // ── Computed ──────────────────────────────────────────────────────────────
        this.tiendaId = () => this.auth.currentUser()?.tienda_id ?? null;
        this.tiendaNombre = () => this.auth.currentUser()?.tienda_nombre ?? null;
    }
    // ── Ciclo de vida ─────────────────────────────────────────────────────────
    ngOnInit() {
        this.pushWelcome();
    ...
    send(text) {
        ...
        this.http.post(this.proxyUrl, payload).pipe(catchError(err => {
            ...
            return of({ respuesta: 'Hubo un problema al conectar con el asistente. Intenta de nuevo.', accion: 'continuar' });
        })).subscribe(res => {
            if (res === null)
                return; // demo mode handles it async
...
__decorate([
    Output()
], DanyChatComponent.prototype, "openChange", void 0);
__decorate([
    ViewChild('messagesRef')
], DanyChatComponent.prototype, "messagesRef", void 0);
DanyChatComponent = __decorate([
    Component({
...
export { DanyChatComponent };
//# sourceMappingURL=dany-chat.component.js.map