...
let TicketDetalleComponent = class TicketDetalleComponent {
    abrirCsat() { this.showCsatModal.set(true); }
    cerrarCsatModal() { this.showCsatModal.set(false); }
    setCsatRespuesta(key, val) {
        this.csatRespuestas.update(r => ({ ...r, [key]: val }));
    }
    constructor(route, router, ticketSvc) {
        this.route = route;
        this.router = router;
        this.ticketSvc = ticketSvc;
        this.ticket = signal(null);
        this.loading = signal(true);
        this.updatingStatus = signal(false);
        this.showRejection = signal(false);
        this.rechazarMotivo = '';
        this.csatScore = signal(0);
        this.comentarioLibre = '';
        this.enviandoComentario = signal(false);
        this.csatComentario = '';
        this.sendingCsat = signal(false);
        this.showRespuesta = signal(false);
        this.respuestaTexto = '';
        this.adjuntoSeleccionado = signal(null);
        // CSAT multi-pregunta
        this.showCsatModal = signal(false);
        this.csatRespuestas = signal({});
        this.csatPreguntas = [
        ...
        this.csatCompleto = computed(() => this.csatPreguntas.every(q => (this.csatRespuestas()[q.key] ?? 0) > 0));
    }
    ngOnInit() {
    ...
    enviarComentario() {
        if (this.enviandoComentario() || !this.comentarioLibre.trim())
            return;
    ...
    responder() {
        if (this.updatingStatus() || !this.respuestaTexto.trim())
            return;
    ...
    setCsat(score) { this.csatScore.set(score); }
    enviarCsat() {
        if (this.sendingCsat() || !this.csatCompleto())
            return;
        ...
        this.ticketSvc.enviarCsat(this.ticket().id, promedio, this.csatComentario || undefined).subscribe({
            next: (t) => {
                this.ticket.set(t);
    ...
    rechazar() {
        if (this.updatingStatus() || !this.rechazarMotivo)
            return;
        this.updatingStatus.set(true);
        this.ticketSvc.update(this.ticket().id, {
            estatus: 'RECHAZADO',
    ...
    accionIcon(a) { return ACCION_ICON[a] ?? '•'; }
    accionLabel(a) {
        const map = {
    ...
    eventosPublicos() {
        return (this.ticket()?.eventos ?? []).filter(ev => !ev.tipo_comentario || ev.tipo_comentario === 'PUBLICO');
    }
