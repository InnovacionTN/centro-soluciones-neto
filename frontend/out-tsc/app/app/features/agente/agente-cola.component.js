...
import { slaStatusColor, origenIcon } from '../../core/models';
const PRIORIDAD_ORDER = {
    CRITICA: 0, ALTA: 1, MEDIA: 2, BAJA: 3,
};
let AgenteColaComponent = class AgenteColaComponent {
    constructor(ticketSvc, auth) {
        this.ticketSvc = ticketSvc;
        this.auth = auth;
        this.tickets = signal([]);
        this.loading = signal(true);
        this.filtroEstatus = signal('');
        this.soloMios = signal(false);
        this.filtroPrioridad = '';
        this.filtroSla = ''; // ← NUEVO Sprint 1
        this.busqueda = '';
        this.countdown = signal(30);
        ...
        this.ticketsMostrados = computed(() => {
            let ts = [...this.tickets()];
            ...
            if (this.soloMios())
                ts = ts.filter(t => t.agente_id === userId);
            if (this.filtroPrioridad)
                ts = ts.filter(t => t.prioridad === this.filtroPrioridad);
            // NUEVO Sprint 1: filtro semáforo SLA
            if (this.filtroSla)
                ts = ts.filter(t => t.sla_status === this.filtroSla);
            if (this.busqueda.trim()) {
                const q = this.busqueda.trim().toLowerCase();
                ts = ts.filter(t => t.folio.toLowerCase().includes(q) ||
            ...
            // luego por prioridad
            const slaOrder = { ROJO: 0, AMARILLO: 1, VERDE: 2, SIN_SLA: 3 };
            return ts.sort((a, b) => {
        ...
        this.grupoNombre = computed(() => {
            ...
            return user?.grupo_nombre ?? (user?.grupo_id ? `Grupo #${user.grupo_id}` : 'Sin grupo');
        });
        this.statusFilters = [
            { label: 'Todos', value: '', count: (c) => 0 },
            { label: 'Sin tomar', value: '__sin_asignar__', count: (c) => c.sin_asignar },
            { label: 'En proceso', value: 'EN_PROCESO', count: (c) => c.en_proceso },
            { label: 'Esp. tienda', value: 'ESPERANDO_TIENDA', count: (c) => c.confirmar },
            { label: '🔴 Vencidos', value: '__vencidos__', count: (c) => c.vencidos },
        ];
    }
    ngOnInit() {
        this.load();
    ...
    origenEmoji(origen) {
        return origenIcon(origen);
    }
};
AgenteColaComponent = __decorate([
    Component({
...
export { AgenteColaComponent };
//# sourceMappingURL=agente-cola.component.js.map