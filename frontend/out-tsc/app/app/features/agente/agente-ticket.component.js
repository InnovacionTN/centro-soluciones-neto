...
let AgenteTicketComponent = class AgenteTicketComponent {
    programarVisita() {
        if (!this.fechaVisita || this.programando())
            return;
    ...
    constructor(route, router, ticketSvc, auth) {
        this.route = route;
        this.router = router;
        this.ticketSvc = ticketSvc;
        this.auth = auth;
        this.ticket = signal(null);
        this.loading = signal(true);
        this.updating = signal(false);
        this.error = signal('');
        this.comentario = '';
        this.esNotaInterna = false;
        this.pendingStatus = signal(null);
        this.showEscalacion = signal(false);
        this.escalando = signal(false);
        this.errorEscalacion = signal('');
        this.grupos = signal([]);
        this.grupoSeleccionado = '';
        this.motivoEscalacion = '';
        this.plantillas = signal([]);
        this.showPlantillas = signal(false);
        this.adjuntoSeleccionado = signal(null);
        this.similares = signal([]);
        this.loadingSimilares = signal(false);
        this.showSimilares = signal(true);
        this.nextActions = () => NEXT_ACTIONS[this.ticket()?.estatus ?? ''] ?? [];
        // Sprint 5A: Mantenimiento
        this.showProgramarForm = signal(false);
        this.fechaVisita = '';
        this.comentarioVisita = '';
        this.programando = signal(false);
        this.isMantenimiento = computed(() => {
            const t = this.ticket();
            if (!t)
                return false;
            // 1. Tipificación del ticket (más confiable si existe)
            const tipArea = t.tipificacion?.area_tecnica ?? '';
            if (tipArea === 'MANTENIMIENTO')
                return true;
            // 2. cat_nivel1 copiado al ticket al crearse
            const nivel1 = (t.cat_nivel1 ?? '').toLowerCase();
            if (nivel1.includes('mantenimiento'))
                return true;
            // 3. Grupo del agente logueado (fallback cuando ticket no tiene tipificación)
            const grupoNombre = (this.auth.currentUser()?.grupo_nombre ?? '').toLowerCase();
            if (grupoNombre.includes('mantenimiento') || grupoNombre.includes('mantto'))
                return true;
            return false;
        });
        this.showProgramarVisita = computed(() => {
            const estatus = this.ticket()?.estatus;
            return this.isMantenimiento() &&
                (estatus === 'EN_PROCESO' || estatus === 'ESPERANDO_PIEZA');
        });
        this.tomarPlantillas = [
            {
                id: 1,
    ...
    ejecutarEscalacion() {
        if (this.escalando() || !this.grupoSeleccionado || this.motivoEscalacion.trim().length < 10)
            return;
        this.escalando.set(true);
        this.errorEscalacion.set('');
        this.ticketSvc.escalar(this.ticket().id, Number(this.grupoSeleccionado), this.motivoEscalacion.trim()).subscribe({
            next: t => {
                this.ticket.set(t);
    ...
    isImg(mime) { return mime?.startsWith('image/') ?? false; }
    dotClass(a) {
        if (a === 'ASIGNACION_AUTO')
            return 'timeline__dot--teal';
        if (a === 'CREACION')
            return 'timeline__dot--blue';
        if (a === 'CAMBIO_ESTADO')
            return 'timeline__dot--amber';
        if (a === 'ESCALACION')
            return 'timeline__dot--amber';
        return '';
    }
...
AgenteTicketComponent = __decorate([
    Component({
...
export { AgenteTicketComponent };
//# sourceMappingURL=agente-ticket.component.js.map