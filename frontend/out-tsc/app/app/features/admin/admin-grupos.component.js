...
let AdminGruposComponent = class AdminGruposComponent {
    constructor(admin) {
        this.admin = admin;
        this.grupos = signal([]);
    ...
    save() {
        if (!this.form.nombre || !this.form.area_tecnica) {
            this.formError.set('Nombre y área son obligatorios');
            return;
        }
        ...
        req.subscribe({
            next: g => {
                if (this.editId()) {
                    this.grupos.update(list => list.map(x => x.id === g.id ? g : x));
                }
                else {
...
AdminGruposComponent = __decorate([
    Component({
...
export { AdminGruposComponent };
//# sourceMappingURL=admin-grupos.component.js.map