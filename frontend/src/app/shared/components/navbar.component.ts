  ...
  template: `
    <aside class="sidebar" [class.sidebar--collapsed]="collapsed()">
      ...
      <nav class="sb-nav">
        ...
        <!-- ── COORDINADOR: acceso a config de su área ── -->
        @if (auth.isCoordinador()) {
          @if (!collapsed()) {
            <span class="sb-sec-label" style="margin-top:18px">
              Mi Área{{ auth.currentUser()?.area_restriccion ? ' — ' + auth.currentUser()?.area_restriccion : '' }}
            </span>
          } @else {
            <div class="sb-divider"></div>
          }
