...
interface TicketMantto {
  ...
  tienda_id: number; tienda_nombre: string | null; agente_nombre: string | null;
  fecha_apertura: string; fecha_visita_programada: string | null;
  pieza_requerida: string | null; sla_status: SlaStatus; sla_porcentaje: number | null;
}
...
export class CoordinadorDashboardComponent implements OnInit {
  ...
  lista = computed(() => {
    let ts = [...this.tickets()];
    const f = this.filtro();
    if (f === '__rojo__') ts = ts.filter(t => t.sla_status === 'ROJO');
    else if (f === '__amarillo__') ts = ts.filter(t => t.sla_status === 'AMARILLO');
    else if (f) ts = ts.filter(t => t.estatus === f);
  ...
  load() {
    ...
    this.http.get<TicketMantto[]>('/api/v1/coordinador/tickets').subscribe({
