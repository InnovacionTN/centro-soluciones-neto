import {
  Component, signal, computed, OnDestroy, inject
} from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { Router } from '@angular/router';
import { Subject, debounceTime, switchMap, catchError, of, Subscription } from 'rxjs';
import { TicketService } from '../../core/services/ticket.service';
import { AuthService } from '../../core/services/auth.service';
import { ClasificacionResponse } from '../../core/models';
...
export class NuevoTicketComponent implements OnDestroy {
  ...
  clasificacion = signal<ClasificacionResponse | null>(null);
  ...
  private classify$ = new Subject<string>();
  private sub: Subscription;

  constructor() {
    this.sub = this.classify$.pipe(
      debounceTime(600),
      switchMap(desc => {
        if (desc.length < 15) {
        ...
        return this.ticket.classify({ descripcion: desc, tienda_id: tiendaId }).pipe(
          catchError((err) => {
      ...
      this.clasificando.set(false);
      if (res) this.clasificacion.set(res);
    });
  }
  ...
  confidenceClass() {
    ...
    if (c >= 70) return 'confidence__fill--high';
    if (c >= 40) return 'confidence__fill--medium';
