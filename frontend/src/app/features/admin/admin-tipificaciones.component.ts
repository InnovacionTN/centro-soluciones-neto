    ...
    <div class="admin-section">
      <div class="section-bar">
        <div class="section-bar-left">
          ...
          <span class="total-badge">{{ tipFiltradas().length }} de {{ tipificaciones().length }}</span>
        </div>
      ...
      <div class="filter-row">
        <button class="filter-btn" [class.filter-btn--active]="filtroArea() === ''" (click)="filtroArea.set('')">
          Todas <span class="filter-count">{{ tipificaciones().length }}</span>
        </button>
      ...
      @if (loading()) {
      ...
      } @else if (tipFiltradas().length === 0) {
        <div class="empty-msg">No hay tipificaciones con los filtros seleccionados.</div>
      } @else {
        <div class="tabla-wrapper">
          <div class="admin-table">
            ...
            @for (t of tipFiltradas(); track t.id) {
...
export class AdminTipificacionesComponent implements OnInit {
  ...
  contarArea(area: string): number {
    return this.tipificaciones().filter(t => t.area_tecnica === area).length;
  }
  ...
  load() {
    ...
    this.admin.getTipificaciones().subscribe({
      next: ts => { this.tipificaciones.set(ts); this.loading.set(false); },
  ...
  openEdit(t: TipAdmin) {
    ...
    this.editId.set(t.id); this.formError.set(''); this.showForm.set(true);
  }

  save() {
    if (!this.form.area_tecnica || !this.form.categoria || !this.form.problema) { this.formError.set('Área, categoría y problema son obligatorios'); return; }
    this.saving.set(true);
    const req = this.editId() ? this.admin.updateTipificacion(this.editId()!, this.form) : this.admin.createTipificacion(this.form);
    req.subscribe({
      next: t => {
        if (this.editId()) { this.tipificaciones.update(list => list.map(x => x.id === t.id ? t : x)); }
        else { this.tipificaciones.update(list => [...list, t]); }
  ...
  urgBadge(u: string) { return ({ CRITICA: 'badge--red', ALTA: 'badge--amber', MEDIA: 'badge--blue', BAJA: 'badge--gray' } as any)[u] ?? 'badge--gray'; }
}
