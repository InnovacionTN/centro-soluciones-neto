...
interface KpiEjecutivo {
    ...
    tickets_por_dia_promedio: number;
    sla_cumplido_pct: number;
    tickets_sin_sla: number;
    tiempo_resolucion_p50_horas: number | null;
    tiempo_resolucion_p90_horas: number | null;
    csat_respuestas: number;
    csat_tasa_respuesta_pct: number;
    csat_satisfaccion_pct: number;
    tickets_origen_dany: number;
    tasa_deflexion_dany_pct: number;
    total_reaperturas: number;
    tasa_reapertura_pct: number;
}
...
interface KpiAgente {
    ...
    tiempo_primera_respuesta_horas: number | null;
    sla_cumplido_pct: number | null;
    csat_promedio: number | null;
    csat_respuestas: number;
    total_escalados: number;
    tasa_reapertura_pct: number | null;
    disponible: boolean;
}

type Nivel = 'ejecutivo' | 'area' | 'grupo' | 'agente';
...
export class AdminKpisComponent implements OnInit {
    ...
    // ── Estado ─────────────────────────────────────────────────────────────────
    nivel = signal<Nivel>('ejecutivo');
    desde = this._haceNDias(30);
    hasta = this._hoy();
    filtroArea = '';
    busquedaAgente = '';
    ...
    // Datos
    ejec = signal<KpiEjecutivo | null>(null);
    tendencia = signal<KpiTendencia[]>([]);
    areas = signal<KpiArea[]>([]);
    grupos = signal<KpiGrupo[]>([]);
    agentes = signal<KpiAgente[]>([]);

    // Loading
    loadingEjec = signal(false);
    loadingArea = signal(false);
    loadingGrupo = signal(false);
    loadingAgente = signal(false);

    niveles = [
        { key: 'ejecutivo' as Nivel, icon: '🏢', label: 'Ejecutivo' },
        { key: 'area' as Nivel, icon: '📂', label: 'Por área' },
        { key: 'grupo' as Nivel, icon: '👥', label: 'Por grupo' },
        { key: 'agente' as Nivel, icon: '👤', label: 'Por agente' },
    ];
    ...
    cargarNivel() {
        switch (this.nivel()) {
            case 'ejecutivo': this.cargarEjecutivo(); break;
            case 'area': this.cargarAreas(); break;
            case 'grupo': this.cargarGrupos(); break;
            case 'agente': this.cargarAgentes(); break;
        }
    }
    ...
    drillArea(area: string) {
        this.filtroArea = area;
        this.setNivel('grupo');
    }

    drillGrupo(_grupoId: number) {
    ...
    cargarEjecutivo() {
        ...
        this.http.get<KpiEjecutivo>('/api/v1/admin/kpis/ejecutivo',
        ...
        this.http.get<KpiTendencia[]>('/api/v1/admin/kpis/tendencia',
            { headers: this.headers(), params: new HttpParams().set('meses', '6') }
        ).subscribe({ next: d => this.tendencia.set(d), error: () => { } });
    }

    cargarAreas() {
        this.loadingArea.set(true);
        this.http.get<KpiArea[]>('/api/v1/admin/kpis/por-area',
            { headers: this.headers(), params: this.params() }
        ).subscribe({
    ...
    cargarGrupos() {
        ...
        let p = this.params();
        if (this.filtroArea) p = p.set('area', this.filtroArea);
        this.http.get<KpiGrupo[]>('/api/v1/admin/kpis/por-grupo',
    ...
    cargarAgentes() {
        ...
        this.http.get<KpiAgente[]>('/api/v1/admin/kpis/por-agente',
            { headers: this.headers(), params: this.params() }
        ).subscribe({
    ...
    exportar() {
        ...
        this.http.post('/api/v1/admin/tickets/exportar',
    ...
    private _haceNDias(n: number): string {
        const d = new Date();
